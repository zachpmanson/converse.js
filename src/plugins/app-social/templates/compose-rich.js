 ...
 * (bold/italic/strikethrough/code were added to the sprite for this composer).
 ...
 */
const fmtButton = (el, type, label, icon) => html`<button
    type="button"
...
export default (el) => html`
    <div class="social-compose social-compose--rich">
        <div class="social-rich">
            ...
            ></div>
            ${el._empty
                ? html`<span class="social-rich__placeholder">${__('What’s on your mind?')}</span>`
                : ''}
        </div>

        <div class="social-rich__toolbar">
            ${fmtButton(el, 'bold', __('Bold'), 'bold')}
            ${fmtButton(el, 'italic', __('Italic'), 'italic')}
