 ...
 *
 * SPIKE: a rich-text Social composer built on Lexical, gated behind the
 * `social_rich_compose` setting (the plain {@link SocialCompose} textarea remains
 * the default). Lexical itself is code-split: it loads via a dynamic `import()` on
 * first focus (see {@link ./lexical-editor.js}), so it never weighs down the
 * core bundle. On submit the post is published as Movim-compatible dual content:
...
// Cap the inline emoji-autocomplete list so a short query (e.g. a single letter)
// stays a tight, keyboard-navigable menu rather than a wall of glyphs.
...
export default class SocialComposeRich extends CustomElement {
    ...
    constructor() {
        ...
        // Inline emoji-shortname autocomplete (open only while suggestions exist).
        ...
        // The query the current suggestions were built for (the chars after the colon).
        this._emoji_query = '';

        // The query the user dismissed with Escape; the menu stays closed for it until
        // the query changes (so a later editor update can't re-open the same trigger).
        /** @type {string|null} */
    ...
    ensureEditor() {
        ...
        // Warm the emoji data alongside the editor, so the shortname autocomplete
        // opens on the very first `:query` rather than one fetch later.
    ...
    /**
     * Close the emoji menu whenever focus leaves the editor: a click elsewhere on
     * the page, tabbing to a toolbar button, or the emoji picker opening. The menu
     * is unusable without editor focus anyway (its keyboard handling lives on the
     * editable). Picking from the menu itself never blurs the editor, because the
     * menu rows prevent the mousedown default, so this only fires for genuine
     * focus moves away from the editable.
     */
    /**
     * @param {FocusEvent} [ev]
     */
