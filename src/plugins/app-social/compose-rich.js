...
// Cap the inline typeahead list so a short query (e.g. a single letter) stays a
// tight, keyboard-navigable menu rather than a wall of rows.
 ...
 * exclusive (each excludes the others' trigger character from its query charset),
 * so at most one source is active at a time.
 */
...
export default class SocialComposeRich extends CustomElement {
    ...
    /**
     * Close the typeahead menu whenever focus leaves the editor: a click elsewhere
     * on the page, tabbing to a toolbar button, or the emoji picker opening. The
     * menu is unusable without editor focus anyway (its keyboard handling lives on
     * the editable). Picking from the menu itself never blurs the editor, because
     * the menu rows prevent the mousedown default, so this only fires for genuine
